...
export default async function ClientDashboard({ params }: { params: Promise<{ clientKey: string }> }) {
  const { clientKey } = await params
  const { targetClientId, client, access } = await requireClientView(clientKey)

  // All reads use the service-role client scoped to the TARGET client:
  // platform admins / org members viewing another client are not covered by
  // RLS on client_twins/connector_credentials/etc., so an anon client would
  // silently return nothing for them.
  const svc = createServiceClient()

  const { data: clientFull } = await svc
    .from('clients')
    .select('*')
  ...
  const { count: agentCount } = await svc
    .from('agents')
    .select('*', { count: 'exact', head: true })
    .eq('is_system_agent', true)

  // ── Business layer: pipeline + connector usage ──
  const orgId = identity?.organization_id ?? clientFull?.organization_id ?? null
  ...
  if (orgId) {
    ...
    const [openDealsRes, wonDealsRes, leadsRes] = await Promise.all([
      svc.from('crm_deals').select('value').eq('organization_id', orgId).eq('status', 'open'),
      svc.from('crm_deals').select('value').eq('organization_id', orgId).eq('status', 'won').gte('closed_at', monthStart.toISOString()),
      svc.from('crm_leads').select('id', { count: 'exact', head: true }).eq('organization_id', orgId).gte('created_at', monthStart.toISOString()),
    ])
  ...
  const [{ data: usageRows }, { data: entRow }] = await Promise.all([
    svc.from('connector_usage_counters').select('metric, count').eq('client_id', targetClientId).eq('period_month', periodStr),
    clientFull?.plan_tier_key
      ? svc.from('tier_entitlements').select('max_dms_per_month, max_emails_per_month').eq('plan_key', clientFull.plan_tier_key).maybeSingle()
      : Promise.resolve({ data: null }),
  ])
  const dmUsed = usageRows?.find((u: any) => u.metric === 'dm')?.count ?? 0
  const emailUsed = usageRows?.find((u: any) => u.metric === 'email')?.count ?? 0
  const packBonus = (clientFull?.connector_pack_quantity ?? 0)
  ...
  const name = clientFull?.full_name ?? identity?.full_name ?? 'Client'
  ...
  const quickActions = [
    { label: 'Open Zuri', href: `${prefix}/zuri`, icon: '◈', desc: 'Personal concierge' },
    { label: 'Essence Intel', href: `${prefix}/essence`, icon: '⊙', desc: 'Daily briefings' },
    { label: 'My Agents', href: `${prefix}/agents`, icon: '⊕', desc: 'Deploy & manage' },
    { label: 'My Swarms', href: `${prefix}/swarms`, icon: '⊗', desc: 'Swarm intelligence' },
  ]
  ...
  return (
    <div className="max-w-6xl mx-auto animate-fade-in">
      ...
      <div className="mb-8">
        ...
        <EssenceBoard userId={targetClientId} userRole={identity?.role ?? 'client'} />
      </div>
        ...
        <div className="lg:col-span-1 space-y-5">
          ...
          <div className="glass rounded-sm p-5 border border-white/[0.06]">
            ...
            <p className="text-sm text-white/50 italic leading-relaxed">
              &ldquo;Intelligence is the ability to adapt to change. What&apos;s one pattern you&apos;ll break today?&rdquo;
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-8">
        <UpgradePanel currentRole={identity?.role ?? 'client'} />
      </div>
    </div>
  )
}
