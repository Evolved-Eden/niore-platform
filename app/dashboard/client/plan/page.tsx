    ...
    <div className="max-w-5xl mx-auto animate-fade-in">
      ...
      <div className="mb-8">
        <h1 className="font-display text-2xl font-bold tracking-tight mb-1">
          Plan & <span className="text-[#C6A664]">Billing</span>
        </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="glass rounded-sm p-6 border border-white/[0.06]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
              <div className="bg-white/[0.03] rounded-sm p-4 border border-white/[0.06]">
                ...
                <Badge
                  ...
                  color={planTier === 'none' ? '#6b7280' : '#C6A664'}
                />
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {additionalPlans.map(p => (
                    <Badge key={p} label={planLabel(p)} color="#8B7AA8" />
                  ))}
                </div>
              </div>
            )}
            ...
            {addons.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {addons.map(a => (
                    <Badge key={a} label={ADDON_DEFS[a]?.name || a} color="#8B7AA8" />
                  ))}
                </div>
              </div>
            )}
          ...
          {isOrgManaged && (
            <div className="glass rounded-sm p-6 border border-white/[0.06] border-l-4" style={{ borderLeftColor: '#5E8B84' }}>
              ...
              <div className="flex items-start gap-4">
                <div className="w-10 h-10 rounded-full bg-white/[0.04] flex items-center justify-center shrink-0 border border-white/[0.06]">
                  <span className="text-lg text-[#5E8B84]">◆</span>
                </div>
                        ...
                        return (
                          <div
                            ...
                            className={`rounded-sm p-3 border transition-all ${
                              isCurrent
                                ? 'border-[#C6A664]/40 bg-[#C6A664]/5'
                                : isActive
                                ? 'border-[#8B7AA8]/30 bg-[#8B7AA8]/5'
                                : 'border-white/[0.06] bg-white/[0.02] hover:border-white/[0.15]'
                            }`}
                          >
                            <div className="flex items-center justify-between mb-1.5">
                              <span className={`text-sm font-medium ${isCurrent ? 'text-[#C6A664]' : isActive ? 'text-[#8B7AA8]' : 'text-white/60'}`}>
                                {tier.name}
                              </span>
                              {isCurrent && (
                                <span className="text-[8px] uppercase tracking-widest text-[#C6A664] bg-[#C6A664]/10 px-1.5 py-0.5 rounded-sm">Active</span>
                              )}
                              {isActive && !isCurrent && (
                                <span className="text-[8px] uppercase tracking-widest text-[#8B7AA8] bg-[#8B7AA8]/10 px-1.5 py-0.5 rounded-sm">+Plan</span>
                              )}
                            </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="glass rounded-sm p-5 border border-white/[0.06]">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex items-center justify-between">
                ...
                <Badge label={planLabel(planTier)} color={planTier === 'none' ? '#6b7280' : '#C6A664'} />
              </div>
              <div className="flex items-center justify-between">
              ...
              {Object.entries(ADDON_DEFS).map(([key, def]) => {
                ...
                return (
                  <div key={key} className={`text-xs p-2.5 rounded-sm border ${
                    ...
                    <div className="flex items-center justify-between">
                      <span className={enabled ? 'text-[#8B7AA8]' : 'text-white/50'}>{def.name}</span>
                      {enabled && <span className="text-[8px] uppercase text-[#8B7AA8]">Enabled</span>}
                    </div>
          ...
          {/* ─── Org Note ──────────────────────────────────────── */}
          {isOrgManaged && (
            <div className="glass rounded-sm p-5 border border-white/[0.06]" style={{ borderLeftColor: '#5E8B84', borderLeftWidth: 2 }}>
