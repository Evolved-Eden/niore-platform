...
export function EssenceEnginePanel() {
  ...
  function downloadReport() {
    ...
    const lines: string[] = []
    lines.push('========================================')
    lines.push('  EVOLVED EDEN — INTELLIGENCE REPORT')
    ...
    a.href = url
    a.download = `intelligence-${new Date().toISOString().slice(0, 10)}.txt`
    a.click()
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold">Essence Engine</h3>
        {(blueprint || lenses) && (
          <button
      ...
      {!blueprint ? (
        /* No Essence Engine yet */
        <div className="glass rounded-sm p-8 text-center">
          <div className="text-4xl mb-4">◈</div>
          <h2 className="text-xl font-semibold mb-2">No Essence Engine Yet</h2>
          <p className="text-white/50 text-sm mb-6 max-w-md mx-auto">
            Your Essence Engine is the foundation of your intelligence system. Take the assessment to map your identity, vision, and capabilities.
          </p>
          ...
          {/* Lens preview (no engine yet but lenses exist) */}
          {lenses && (
          ...
          >
            {intake.hasIntake ? 'Complete Assessment →' : 'Start Assessment →'}
          </Link>
        </div>
      ) : (
        <>
          ...
          <div className="glass rounded-sm p-6">
            <div className="flex items-start justify-between mb-6">
              <div>
                <div className="text-xs text-white/30 tracking-widest uppercase">Intelligence Score</div>
          ...
          {/* ══════ Essence Engine — Multi-Lens Intelligence ══════ */}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-white/70">
                  {twinExists ? 'Your AI Twin is active and linked to your intelligence.' : 'No AI Twin deployed yet.'}
                </p>
                {!twinExists && (
                  <Link href={`${prefix}/twin`} className="text-xs text-[#C6A664] hover:underline mt-1 inline-block">
                    Configure Twin from Intelligence →
                  </Link>
                )}
              </div>
            </div>
      ...
      {/* ── Expanded Essence Engine Upgrade ── */}
      <div className={`glass rounded-sm p-6 border ${purchasedExpanded ? 'border-[#C6A664]/30' : 'border-white/[0.06] hover:border-white/15'}`}>
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <h3 className="text-lg font-semibold">Expanded Essence Engine</h3>
              {purchasedExpanded && (
        ...
        {!purchasedExpanded && (
          ...
          >
            {checkoutLoading === 'expanded_blueprint' ? 'Processing...' : 'Purchase Expanded Essence Engine'}
          </button>
        )}
      </div>

      {/* ── Enhanced Essence Engine $35 ── */}
      <div className={`glass rounded-sm p-6 border ${purchasedEnhanced ? 'border-[#5E8B84]/30' : 'border-white/[0.06] hover:border-white/15'}`}>
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <h3 className="text-lg font-semibold">Enhanced Essence Engine</h3>
              {purchasedEnhanced && (
        ...
        {!purchasedEnhanced && (
          ...
          >
            {checkoutLoading === 'enhanced_blueprint' ? 'Processing...' : 'Purchase Enhanced Essence Engine — $35'}
          </button>
        )}
        {purchasedEnhanced && !purchasedExpanded && (
          <p className="mt-3 text-xs text-[#5E8B84]/60 text-center">
            Add the Expanded Essence Engine for a full whole-life intelligence scan → <Link href={`${prefix}/essence-profile/assess`} className="underline hover:text-[#5E8B84]">Go to Assessment</Link>
          </p>
        )}
      </div>
      ...
      <div className="glass rounded-sm p-6 border border-white/[0.06]">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-white/50">Add specific life-domain intelligence assessments to your Essence Engine. Each module unlocks 5 questions in that domain.</p>
          </div>
          <div className="text-right">
