/**
 * Evolved Eden — Stripe Product & Price Setup
...
async function run() {
  console.log('\n🔧 Creating Evolved Eden Stripe products...\n')

  // FOUNDER
  const founder = await stripe.products.create({
    name: 'Evolved Eden — Founder',
  ...
  for (const a of addons) {
    const p = await stripe.products.create({ name: `Evolved Eden — ${a.name}` })
