/**
 * Deploy ALL 5 n8n workflows — v2 (no Discord nodes, clean)
 * Run: node scripts/deploy-v2.mjs
 */
...
const WF1 = joinLines([
  ...
  `  type: 'n8n-nodes-base.postgres', version: 2,`,
  `  config: {`,
  `    name: 'Count Pending',`,
  `    parameters: { operation: 'executeQuery', query: expr('SELECT COUNT(*)::int AS pending_count\\nFROM public.workflow_jobs\\nWHERE status = \\'pending\\';') },`,
  ...
  `    name: 'Call Worker',`,
  `    parameters: {`,
  `      method: 'POST', url: 'https://jebixydqpvsegvrtfmgm.supabase.co/functions/v1/workflow-worker',`,
...
const WF2 = joinLines([
  ...
  `    parameters: { operation: 'executeQuery', query: expr("SELECT *\\nFROM public.workflow_schedules\\nWHERE active = true\\n  AND next_run_at <= NOW()\\nORDER BY next_run_at\\nLIMIT 500;") },`,
  `    credentials: { postgres: newCredential('Postgres account') },`,
  `    position: [450, 300],`,
  `  },`,
  ...
  `});`,
  ``,
  `const splitBatches = splitInBatches({ version: 3, config: { name: 'Split Batches', parameters: { batchSize: 50 }, position: [650, 300] } });`,
  ``,
  ...
  `    parameters: {`,
  `      method: 'POST', url: 'https://jebixydqpvsegvrtfmgm.supabase.co/functions/v1/workflow-trigger',`,
  `      authentication: 'predefinedCredentialType', nodeCredentialType: 'httpHeaderAuth',`,
  `      sendHeaders: true,`,
  `      headerParameters: { parameters: [{ name: 'Authorization', value: expr('Bearer {{ $env.SUPABASE_SERVICE_ROLE_KEY }}') }, { name: 'Content-Type', value: 'application/json' }] },`,
  ...
  `    parameters: { operation: 'executeQuery', query: expr("UPDATE public.workflow_schedules\\nSET last_run_at = NOW(),\\nnext_run_at = public.calculate_next_run(cron_expression)\\nWHERE id = '{{ $json.id }}'::uuid;") },`,
  `    credentials: { postgres: newCredential('Postgres account') },`,
  `    position: [1050, 300],`,
  `  },`,
  `  output: [{ updated: true }]`,
...
const WF3 = joinLines([
  ...
  `});`,
  ``,
  `const splitBatches = splitInBatches({ version: 3, config: { name: 'Split Batches', parameters: { batchSize: 25 }, position: [650, 300] } });`,
  ``,
  ...
  `  config: {`,
  `    name: 'Mark Processed',`,
  `    parameters: { operation: 'executeQuery', query: expr("UPDATE public.workflow_dead_letters\\nSET processed = true,\\nprocessed_at = NOW()\\nWHERE id = '{{ $json.id }}'::uuid;") },`,
  `    credentials: { postgres: newCredential('Postgres account') },`,
  `    position: [1250, 300],`,
  `  },`,
  `  output: [{ updated: true }]`,
  ...
  `      .onFalse(markProcessed.to(nextBatch(splitBatches)))`,
  `    )`,
  `  );`,
]);
...
const WF4 = joinLines([
  ...
  `  config: {`,
  `    name: 'Upsert Metrics',`,
  `    parameters: { operation: 'executeQuery', query: expr("INSERT INTO public.workflow_metrics (organization_id, workflow_id, total_runs, completed_runs, failed_runs, avg_duration, updated_at)\\nVALUES (\\n  '{{ $json.organization_id }}'::uuid,\\n  '{{ $json.workflow_id }}',\\n  {{ $json.total_runs }},\\n  {{ $json.completed_runs }},\\n  {{ $json.failed_runs }},\\n  {{ $json.avg_duration }},\\n  NOW()\\n)\\nON CONFLICT (organization_id, workflow_id)\\nDO UPDATE SET\\n  total_runs = EXCLUDED.total_runs,\\n  completed_runs = EXCLUDED.completed_runs,\\n  failed_runs = EXCLUDED.failed_runs,\\n  avg_duration = EXCLUDED.avg_duration,\\n  updated_at = NOW();") },`,
  `    credentials: { postgres: newCredential('Postgres account') },`,
  `    position: [850, 300],`,
  `  },`,
...
const WF5 = joinLines([
  ...
  `  type: 'n8n-nodes-base.webhook', version: 1.2,`,
  `  config: { name: 'Webhook', parameters: { path: 'replay-workflow', httpMethod: 'POST', responseMode: 'lastNode', options: {} }, webhookId: 'replay-workflow', position: [250, 300] },`,
  ...
  `    parameters: { respondWith: 'json', responseBody: expr('{{ { "success": true, "message": "Workflow replayed successfully", "workflow_run_id": $json.workflow_run_id, "workflow_id": $json.workflow_id } }}') },`,
  `    position: [1050, 150],`,
  `}});`,
  ``,
  ...
  `    parameters: { respondWith: 'json', responseStatusCode: 404, responseBody: expr('{{ { "success": false, "message": "No checkpoint found for workflow_run_id", "workflow_run_id": $json.workflow_run_id } }}') },`,
  `    position: [850, 500],`,
  `}});`,
  ``,
...
const WORKFLOWS = [
  { id: 'wf1', code: WF1, name: 'WF1 — Queue Poller',
    desc: 'Polls workflow_jobs every 5s, calls worker edge function at supabase/functions/v1/workflow-worker' },
  ...
  { id: 'wf3', code: WF3, name: 'WF3 — Dead Letter Handler',
    desc: 'Every 5m processes workflow_dead_letters, retries retryable ones via edge function' },
...
for (const wf of WORKFLOWS) {
  ...
  if (valJson.warnings?.length > 0) {
    console.log(`   ⚠️  ${valJson.warnings.length} warnings:`);
  ...
  ok++;
  console.log(`   ✅ Created!`);
  if (createJson.workflowId) console.log(`   ID: ${createJson.workflowId}`);
  ...
  if (createJson.workflowId) {
    ...
    if (pubJson.activated === true) {
      console.log(`   ✅ Published & activated!`);
    } else if (pubJson.activated === false) {
      console.log(`   ✅ Published (not activated)`);
    } else {
